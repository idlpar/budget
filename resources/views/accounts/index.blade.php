            ...
            <div class="bg-white dark:bg-slate-700 shadow-lg rounded-lg overflow-hidden">
                ...
                <div class="px-4 py-3 flex justify-center gap-2">
                    <form method="GET" action="{{ route('accounts.index') }}" class="flex gap-2">
                        ...
                        <button type="submit" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 dark:hover:bg-indigo-500">Search</button>
                    </form>
                </div>
                <!-- Filter Inputs -->
                <div class="pl-4 pr-0 py-3 flex gap-2">
                    <form id="filter-form" method="GET" action="{{ route('accounts.index') }}" class="w-full flex gap-2 flex-wrap">
                        ...
                        <div class="flex items-center gap-2">
                            <label for="per_page" class="text-sm text-slate-700 dark:text-slate-300">Show:</label>
                            <select id="per_page" name="per_page" class="w-24 px-4 py-2 text-sm text-slate-900 dark:text-slate-200 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-md focus:ring-indigo-500 focus:border-indigo-500" onchange="this.form.submit()">
                ...
                <div class="border-t border-slate-200 dark:border-slate-600">
                    <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-600">
                        ...
                        @forelse ($accountHeads as $index => $accountHead)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-900 dark:text-slate-200">{{ $accountHeads->firstItem() + $index }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-900 dark:text-slate-200">{{ ucfirst($accountHead->type) }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex items-center gap-3">
                                    <!-- View Icon -->
                                    <a href="{{ route('accounts.show', $accountHead) }}" title="View Account Head" class="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300">
                                    ...
                                    <!-- Edit Icon -->
                                    <a href="{{ route('accounts.edit', $accountHead) }}" title="Edit Account Head" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300">
                ...
                <div class="px-4 py-3 border-t border-slate-200 dark:border-slate-600">
                    {{ $accountHeads->appends(request()->except('page'))->links() }}
                </div>
                <!-- Total Heads Count -->
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const filterInputs = document.querySelectorAll('#filter-form input:not([name="per_page"]), #filter-form select:not([name="per_page"])');
