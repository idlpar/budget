        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    <form method="GET" action="{{ route('reports.expense') }}" class="mb-6">
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <div>
                                <label for="category" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Category</label>
                                <select name="category" id="category" class="mt-1 block w-full rounded-md border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-gray-100">
                                    <option value="account_head" {{ $category === 'account_head' ? 'selected' : '' }}>Account Head</option>
                                    <option value="department" {{ $category === 'department' ? 'selected' : '' }}>Department</option>
                                    <option value="section" {{ $category === 'section' ? 'selected' : '' }}>Section</option>
                                    <option value="division" {{ $category === 'division' ? 'selected' : '' }}>Division</option>
                                    <option value="transaction" {{ $category === 'transaction' ? 'selected' : '' }}>Transaction</option>
                                </select>
                            </div>
                            <div>
                        ...
                        <div class="overflow-x-auto print:overflow-visible">
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                                ...
                                @foreach($expenses as $expense)
                                    @foreach($expense->transaction->entries as $entry)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                                <a href="{{ route('reports.account-head', $entry->accountHead) }}" class="text-blue-600 hover:underline">{{ $entry->accountHead->name }}</a>
                                            </td>
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-3 text-left text-sm font-medium text-gray-900 dark:text-gray-100">
                                        Total Rows: {{ $totalRows }}<br>
                                        @foreach($totalByHead as $head => $amount)
                                            {{ $head }}: {{ $amount }} BDT<br>
                                        @endforeach
                                        Total Expenditure: {{ $totalExpenditure }} BDT
                                    </td>
                                </tr>
                                </tfoot>
                            </table>
